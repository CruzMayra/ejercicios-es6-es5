...
/*var totalByCategory = function(array, category) {
  ...
  var filtrando = array.filter(filterCategory).reduce(sumInventory, 0);
...
const totalByCategory = (array, category) => {
  ...
  let filtrando = array.filter(filterCategory).reduce(sumInventory, 0);
  return filtrando;
};
